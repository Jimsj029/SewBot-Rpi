...
class PatternMode:
    ...
    def _build_skeleton_path(self, pattern_alpha, actual_w, actual_h):
        """Build a movement path by column-snake scanning all pattern pixels.

        Sweeps column by column left→right, alternating top→bottom and
        bottom→top each column.  For stroke-based patterns (V, W, U, n …) this
        naturally traces: down the left arm, up/across, down the next arm, etc.,
        guaranteeing 100 % pattern coverage without any skeleton thinning.
        ...
        pat_crop = pattern_alpha[:actual_h, :actual_w]
        ys_arr, xs_arr = np.where(pat_crop > self.pattern_alpha_threshold)
        if ys_arr.size == 0:
            return None

        # Group all pattern pixels by column
        cols: dict = {}
        for y, x in zip(ys_arr.tolist(), xs_arr.tolist()):
            cols.setdefault(x, []).append(y)

        # Snake scan: each column alternates direction
        path_yx = []
        going_down = True
        for col_x in sorted(cols.keys()):
            col_ys = sorted(cols[col_x])
            if not going_down:
                col_ys = col_ys[::-1]
            for y in col_ys:
                path_yx.append((y, col_x))
